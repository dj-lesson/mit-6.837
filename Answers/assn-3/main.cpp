...
namespace
{
  ...
  void initSystem(int argc, char * argv[])
  {
	...
	//initialize system. 0 for SimpleSystem, 1 for PendulumSystem, 2 for ClothSystem.
	if (which_system == 0) {
		system = new SimpleSystem();
	}
	else if (which_system == 1) {
		system = new PendulumSystem(4);
	}
	else if (which_system == 2) {
		system = new ClothSystem();
	}
  ...
  void drawSystem()
  {
    ...
    void keyboardFunc( unsigned char key, int x, int y )
    {
		...
		case 'w':
		{
			system->switch_draw_spring();
			break;
		}
		case 's':
		{
			system->switch_move_open();
			break;
		}
		case 'r':
		{
			...
			initSystem(argcount, argvalues);
			break;
		}
		case 't':
		{
			delete system;
			if (which_system == 0) which_system = 1;
			else if (which_system == 1) which_system = 2;
			else if (which_system == 2) which_system = 0;
			initSystem(argcount, argvalues);
			break;
		}
        default:
